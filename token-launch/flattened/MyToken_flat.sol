...
/**
 * @title MyToken
 * @dev ERC20 token with pausable transfers and blacklisting capability.
 * @notice This token implements:
 *         - Standard ERC20 functionality with pausable transfers
 *         - Owner-based access control
 *         - Blacklist to freeze specific addresses
 *         - Initial supply minting to deployer
 *         - Timelock for critical operations (pause, unpause, blacklist, unBlacklist)
 */
contract MyToken is ERC20, ERC20Pausable, Ownable {
    /// @notice Maximum total supply (100 billion tokens * 10^18)
    uint256 public constant MAX_SUPPLY = 10000000000 * 10**18;

    /// @notice Timelock delay for critical operations (24 hours)
    uint256 public constant TIMELOCK_DELAY = 24 hours;

    /// @notice Blacklisted addresses cannot transfer tokens
    mapping(address => bool) public isBlacklisted;

    /// @notice Addresses authorized for burnFrom (owner can only burn from these)
    mapping(address => bool) public authorizedBurnFrom;

    /// @notice Pending timelock actions - actionId => executeAt timestamp
    mapping(bytes32 => uint256) public pendingActions;

    /// @notice Track pending operation type per target address
    mapping(address => bytes32) public pendingOperationForAddress;

    /// @notice Track pending operation type (no address)
    bytes32 public pendingGlobalOperation;
    ...
    /// @notice Emitted when address is authorized for burnFrom
    event AuthorizedForBurn(address indexed account);

    /// @notice Emitted when address is deauthorized for burnFrom
    event DeauthorizedForBurn(address indexed account);

    /// @notice Emitted when tokens are burned
    event Burned(address indexed from, uint256 amount);

    /// @notice Emitted when a timelocked action is scheduled
    event TimelockScheduled(bytes32 indexed actionId, uint256 executeAt);

    /// @notice Emitted when a timelocked action is executed
    event TimelockExecuted(bytes32 indexed actionId);

    /// @notice Emitted when a timelocked action is cancelled
     ...
     * @param initialSupply Initial supply to mint to deployer (in tokens, not wei)
     * @param initialOwner Initial owner address for Ownable
     */
    constructor(
        string memory name_,
    ...
    /**
     * @dev Modifier to check if timelock delay has passed
     */
    ...
    /**
     * @dev Schedule a timelocked action. Returns action ID.
    ...
    /**
     * @dev Execute a timelocked action after delay
     * @param actionId Unique identifier for the action
     */
    ...
    /**
     * @dev Cancel a pending timelock action
     * @param actionId Unique identifier for the action
     */
    ...
    /**
     * @dev Pause all token transfers. Only owner can call. Requires timelock.
     */
    function pause() external onlyOwner {
        bytes32 actionId = keccak256(abi.encodePacked("pause"));
        _scheduleTimelockAction(actionId);
        pendingGlobalOperation = "pause";
    }

    /**
     * @dev Execute pause after timelock. Only owner can call.
     */
    function executePause() external onlyOwner {
        bytes32 actionId = keccak256(abi.encodePacked("pause"));
        _executeTimelockAction(actionId);
        pendingGlobalOperation = bytes32(0);
        _pause();
    }

    /**
     * @dev Unpause all token transfers. Only owner can call. Requires timelock.
     */
    function unpause() external onlyOwner {
        bytes32 actionId = keccak256(abi.encodePacked("unpause"));
        _scheduleTimelockAction(actionId);
        pendingGlobalOperation = "unpause";
    }

    /**
     * @dev Execute unpause after timelock. Only owner can call.
     */
    ...
    /**
     * @dev Add address to blacklist. Only owner can call. Requires timelock.
    ...
    function blacklist(address account) external onlyOwner {
        ...
        _scheduleTimelockAction(actionId);
        pendingOperationForAddress[account] = "blacklist";
    }

    /**
     * @dev Execute blacklist after timelock. Only owner can call.
     * @param account Address to blacklist
     */
    function executeBlacklist(address account) external onlyOwner {
        ...
        bytes32 actionId = keccak256(abi.encodePacked("blacklist", account));
        _executeTimelockAction(actionId);
        pendingOperationForAddress[account] = bytes32(0);
        isBlacklisted[account] = true;
        emit Blacklisted(account);
    }

    /**
     * @dev Remove address from blacklist. Only owner can call. Requires timelock.
     * @param account Address to remove from blacklist
     */
    function unBlacklist(address account) external onlyOwner {
        bytes32 actionId = keccak256(abi.encodePacked("unBlacklist", account));
        _scheduleTimelockAction(actionId);
        pendingOperationForAddress[account] = "unBlacklist";
    }

    /**
     * @dev Execute unBlacklist after timelock. Only owner can call.
     * @param account Address to remove from blacklist
     */
     ...
     * @param amount Amount of tokens to burn (in tokens, not wei)
     */
    ...
    /**
     * @dev Authorize an address for owner to burn from. Only owner can call.
    ...
    /**
     * @dev Deauthorize an address for owner to burn from. Only owner can call.
    ...
    /**
     * @dev Burn tokens from authorized address. Only owner can call.
     * @param account Address to burn from (must be authorized)
